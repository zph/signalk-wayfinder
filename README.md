...
| `alternativeCount`          | 5       | Number of distinct routes requested and ranked for each calculation. Binnacle can override this per passage.                                                                                                                          |
| `alternativeWorkerCount`    | 4       | Maximum worker threads used concurrently for alternative attempts. Increase for faster planning when memory permits; each worker loads its own weather and shoreline data.                                                            |
